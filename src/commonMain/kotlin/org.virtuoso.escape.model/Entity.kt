...
class Entity(val id: String, val states: Map<String, EntityState>, var currentState: String) {
     ...
     * @param entityStates The states the entity has, the default state will be a state with the name of the entity id
     ...
     */
    constructor(id: String, attackAction: ActionType?, inspectAction: ActionType?, interactAction: ActionType?, inputAction: InputActionType?) : this(
        id, mapOf(id to EntityState(id, attackAction, inspectAction, interactAction, inputAction)), id
    )
    ...
    fun swapState(newState: String) {
        require(states.containsKey(newState)) { "Entity '${this.id}' cannot be swapped to non-existent state: '${newState}'" }
