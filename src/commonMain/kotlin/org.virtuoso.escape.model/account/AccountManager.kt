...
data class AccountManager(val accounts: Map<Uuid, Account>, val gameStates: Map<Uuid, GameState>) {
     ...
     * @param username The username to attempt.
     * @param password The password to attempt.
     * @return the `Optional<Account>` if no user exists with this exact username-password combination, otherwise
     * `Optional.empty`.
     */
    ...
    fun invalidLoginInfo(username: String, password: String): String {
        ...
        return if (usernameCount == 0 && passwordCount == 0) "Both username and password input is invalid."
        else if (usernameCount > 0) "Password input is invalid."
        else "Username input is invalid."
    }
    ...
    fun accountExists(username: String, password: String): Account? {
        return accounts.entries.firstOrNull { (_, v) -> v.username == username && v.hashedPassword == hashPassword(password) }?.value;
    }
}
