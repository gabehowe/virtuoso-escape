...
class ActionTests {
  ...
  fun addPenaltyTest() {
    ...
    for ((diff, sev, expectedDiff) in cases) {
      ...
      // "Penalize the player by removing time" -> usually implies increasing penalty or decreasing
      // time?
      ...
      // Wait, if penalty is accumulated negative score, then subtracting points makes it more
      // negative (worse)?
  ...
  @Test
  fun chainTest() {
    val action = Actions.chain(Actions.setMessage("initial"), Actions.setMessage("expected"))
    action(proj.state)
  ...
  fun conditionalTest() {
    // True case
    Actions.conditional({ true }, Actions.setMessage("true"), Actions.setMessage("false"))(
        proj.state
    )
    assertEquals("true", proj.state.message)

    // False case
    Actions.conditional({ false }, Actions.setMessage("true"), Actions.setMessage("false"))(
        proj.state
    )
  ...
  fun takeInputTest() {
    val action =
        Actions.takeInput(
            listOf("one" to Actions.setMessage("one"), "two" to Actions.setMessage("two")),
