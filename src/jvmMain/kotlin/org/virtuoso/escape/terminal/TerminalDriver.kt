...
class TerminalDriver {
     ...
     */
    fun makeTuiActionMap(vararg input: Pair<FunString, () -> Unit>): MutableList<Pair<FunString, () -> Unit>> {
     ...
     */
    fun tryLogin(
        authenticator: (String, String) -> Boolean, checkAuthError: (String, String) -> String
    ): Boolean {
        val username: String
        val password: String
        val flag: Boolean
        ...
        if (!flag) {
            display(
                (FunString(checkAuthError(username, password))).red().toString()
            )
        }
    ...
    fun display(display: String?, vararg args: Any?) {
        ...
        if (args.isEmpty()) println(display)
    ...
    private fun getProgressBar(projection: GameProjection): String {
        ...
        // finds floor
        val currentFloorIndex = (0 until totalFloors).firstOrNull { i ->
    ...
    fun menu_ending(projection: GameProjection) {
        ...
        val hintsListDisplay = if (totalHintsUsed > 0) hintsUsedMap.entries.joinToString(
        ...
        // num hints
        val hintmsg = String.format(projection.state.language.string("credits", "hints"), totalHintsUsed)
    ...
    fun menu_changeRoom(projection: GameProjection) {
        ...
        for (room in projection.currentFloor().rooms) {
            actions.add(FunString(projection.language.string(room.id, "name")).bold() to { projection.pickRoom(room) })
        }
    ...
    fun menu_roomActions(projection: GameProjection) {
        ...
        for (e in projection.currentRoom().entities) {
            actions.add(FunString(e.string(projection.language, "name")).italic() to { projection.pickEntity(e) })
        }
    ...
    fun menu_entityAction(projection: GameProjection) {
        val prompt = projection.currentEntity()!!::string.let { { j: String -> it(projection.language, j) } }.let {
            FunString(it("name")).italic() + "\n" + it("introduce")
        }
        val actions = makeTuiActionMap()
        val capabilities = projection.capabilities()
        if (capabilities.interact) actions.add(FunString("Interact").blue() to { projection.interact() })
        ...
        if (capabilities.input) actions.add(
            FunString("Speak").purple() to {
                projection.input(validateInput("What would you like to say? ") { true })
            })
        if (projection.currentItems().isNotEmpty()) actions.add(FunString("Items") to { this.displayItems(projection) })
    ...
    fun menu_difficulty(projection: GameProjection) {
        ...
        for (diff in Difficulty.entries) {
            actions.add(
                FunString(diff.name).terminalColor(diff.ordinal + 196) to { projection.setDifficulty(diff) })
        }
        actions.add(FunString("Nevermind") to {})
    ...
    fun menu_debug(projection: GameProjection) {
        val actions = makeTuiActionMap(FunString("Switch floor") to { menu_debugSwitchFloor(projection) })
    ...
    fun main() {
        ...
        var flag = false
        while (!flag) {
            val actions = makeTuiActionMap(FunString("Login") to {
                flag = tryLogin({ username, password -> projection.login(username, password) }, { username, password ->
                    projection.accountManager.invalidLoginInfo(
                        username, password
                    )
                })
                if (flag) menu_summary(projection)
            }, FunString("Create Account") to {
                flag = tryLogin({ username, password ->
                    projection.createAccount(username, password)
