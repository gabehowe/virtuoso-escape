...
function init() {
    debug = {
        'enabled': false,
        'selected': () => Array.from(document.querySelectorAll('.selected')).map(it => it.id).join("; "),
...
function displaySettings(name) {
    ...
    switch (name) {
        case Settings.SETTINGS: {
            let diff = makeLogicalButton("setting-change-difficulty", "Change Difficulty", true);
            let toggleTTS = makeLogicalButton("setting-toggle-tts", "Toggle TTS", true);
        ...
        case Settings.DEBUG: {
            let changeFloor = makeLogicalButton("debug-change-floor", "Change Floor", true);
            let endGame = makeLogicalButton('debug-end-game', "End game", true)
            let enableDebugMenu = makeLogicalButton('debug-enable-menu', "Debug Menu", true)
        ...
        case Settings.DIFFICULTY: {
            ...
            for (let i of difficulties) {
                let difficultyButton = makeLogicalButton('change-difficulty' + difficulties, i, true);
...
function createKeys() {
    ...
    for (let k in keyMap){
        ...
        elem.innerHTML = ""
        elem.append("[", sourceKey.substring(0, index), new DOMParser().parseFromString(`<u><b>${visualKey}</b></u>`, 'text/html').body.firstElementChild, sourceKey.substring(index + 1), "]")
...
function populateBackground(current, entities) {
    ...
    if (predefined.length > 0 && Array.from(predefined).map(it => it.src).every(it => Array.from(entities).some(key => it.includes(key)))) {
        document.querySelectorAll('#background-entities > img.selected').forEach(it => it.classList.remove('selected'))
        document.querySelector(`#background-entities > img[src*="${current}"]`)?.classList?.add('selected')
        return
    }
