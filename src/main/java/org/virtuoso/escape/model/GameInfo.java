...
public record GameInfo(Map<String, Map<String, String>> language, List<Floor> building) {
    ...
    private Floor acornGrove() {
        ...
        Entity portal_squirrel = new Entity(
                "portal_squirrel",
                new Default(),
                new Default(),
                new Chain(new CompletePuzzle("portal"), new SetFloor(1)),
                null);
        Entity narrator = makeNarrator("acorn_grove");
        Room acornGrove_0 = new Room(
                "acorn_grove_0",
                new ArrayList<>(List.of(intro_squirrel, portal_squirrel, narrator)),
                this.string("acorn_grove_0", "introduce"));
    ...
    private Floor floor1() {
        ...
        EntityState hummus_trash_can = new EntityState(
                "trash_can",
                new Chain(
                        new GiveItem(Item.sealed_clean_food_safe_hummus),
                        new CompletePuzzle("trash"),
                        new SwapEntities("trash_can", "sans_hummus_trash_can")),
                new Default(),
                new Default(),
                null);
        EntityState sans_hummus_trash_can =
                new EntityState("sans_hummus_trash_can", new Default(), new Default(), new Default(), null);
        Entity trash_can = new Entity("trash_can", hummus_trash_can, sans_hummus_trash_can);

        Entity joeHardy = joeHardy();

        EntityState hummus_elephant = new EntityState(
                "elephant_in_the_room",
                new Default(),
                new Default(),
                new Chain(
                        new GiveItem(Item.sunflower_seed_butter),
                        new CompletePuzzle("elephant"),
                        new SwapEntities("elephant_in_the_room", "sans_butter_elephant")),
                null);
        EntityState sans_butter_elephant =
                new EntityState("sans_butter_elephant", new Default(), new Default(), new Default(), null);
        Entity elephant = new Entity("elephant_in_the_room", hummus_elephant, sans_butter_elephant);

        Room room_1400 = new Room(
                "storey_i_0",
                new ArrayList<>(List.of(joeHardy, trash_can, elephant, door, narrator)),
                this.string("storey_i_0", "introduce"));

        Entity almanac = makeAlmanacs(5);
        Room janitor_closet = new Room(this, "storey_i_1", almanac);

        Entity securityBread = new Entity(
                "security",
                new Default(),
                new Default(),
                new Default(),
                new TakeInput(
                        ...
                        TakeInput.makeCases(
                                ".*(?<!w)right.*",
                                new Chain(
                                        new SetMessage(this, "security", "right_answer"),
                                        new GiveItem(Item.right_bread),
                                        new CompletePuzzle("right")),
                                ".*",
                                new Chain(
                                        new AddPenalty(Severity.LOW),
    ...
    private Entity joeHardy() {
        ...
        EntityState introJoe = new EntityState(
                ...
                new Default(),
                new Default(),
                new Chain(new CompletePuzzle("sandwich"), new SwapEntities("joe_hardy", "sans_sandwich_joe")),
                null);
    ...
    private Action turnPage(int flips, int currentPage, int maxFlips, int correctPage) {
        Action swap = flips > 1
                ? new SwapEntities("almanac", "almanac_" + (flips - 1))
        ...
        Function<String, Entity> createDialogueDoorChain = (id) -> {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                states[i] = new EntityState(
                        stateId, setMsg.apply("attack", stateId), setMsg.apply("inspect", stateId), failDoor, null);
            }
    ...
    private Entity createDoorChain(int length, Action shuffle, IntConsumer setDoors) {
        ...
        BiFunction<String, String, Action> sm =
                (stringId, state) -> new SetMessage(this.searchString(stringId, state, "door1"));
    ...
    private Floor floor3() {
        ...
        EntityState box_open = new EntityState(
                "box_open",
                new Default(),
                new Default(),
                new Chain(puzzleMsg.apply("solved"), new GiveItem(Item.keys), new CompletePuzzle("boxes")),
                null);

        var boxLogicSuccess = new TakeInput(
                "",
                TakeInput.makeCases(
                        "(?:box )?open",
        ...
        var boxLogicFollow = new TakeInput(
                ...
                TakeInput.makeCases(
                        "(?:box )?follow",
                        new Chain(new SwapEntities("box_riddle", "box_success"), puzzleMsg.apply("step_follow")),
                        ".*",
                        new Chain(puzzleMsg.apply("step_wrong"), new SwapEntities("box_riddle", "box_start"))));
        EntityState box_step1 =
                new EntityState("box_step1", new Default(), new Default(), new Default(), boxLogicFollow);

        var boxLogicStart = new TakeInput(
                "",
                TakeInput.makeCases(
                        "(?:box )?start",
                        new Chain(new SwapEntities("box_riddle", "box_step1"), puzzleMsg.apply("step_start")),
                        ".*",
                        puzzleMsg.apply("step_wrong")));
        EntityState box_start = new EntityState(
                "box_start",
                puzzleMsg.apply("attack"),
                puzzleMsg.apply("inspect"),
                puzzleMsg.apply("interact"),
                boxLogicStart);
        ...
        Action goToFloor4 = new Chain(doorMsg.apply("unlocked_success"), new SetFloor(4));
        EntityState doorUnlocked = new EntityState(
                "exit_door_unlocked", new Default(), doorMsg.apply("inspect_unlocked"), goToFloor4, null);

        Action checkKeysAndSwap = new Conditional(
                () -> GameState.instance().hasItem(Item.keys),
                new Chain(doorMsg.apply("unlocked_msg"), new SwapEntities("exit_door", "exit_door_unlocked")),
                doorMsg.apply("locked_msg"));
        EntityState doorLocked = new EntityState(
                "exit_door_locked", new Default(), doorMsg.apply("inspect_locked"), checkKeysAndSwap, null);

        Entity exitDoor = new Entity("exit_door", doorLocked, doorUnlocked);

        Room boxRoom = new Room(
                "storey_iii_0",
                new ArrayList<>(List.of(sparrowAmbassador, puzzleBox, exitDoor, narrator)),
                this.string("storey_iii_0", "introduce"));
    ...
    private Floor floor4() {
        ...
        Entity sock_squirrel = new Entity(
                "sock_squirrel",
                new Chain(new CompletePuzzle("unblock"), new SwapEntities("computty", "computty_unblocked")),
                new Default(),
                new Default(),
                null);
        EntityState microwave_blocked =
                new EntityState("microwave_blocked", new Default(), new Default(), new Default(), null);
        // Whoops! JEP 126!
        EntityState microwaveUnblocked =
    ...
    private Entity makeComputtyLogic() {
        ...
        var computtyTarLogic = new TakeInput(
                "",
                TakeInput.makeCases(
                        "rotx 16 code",
                        new Chain(
                        ...
                        "rotx 16 .*",
                        ttyStr.apply("no_file"),
                        "rotx \\d+.*",
                        ttyStr.apply("failed_rotx"),
                        "rotx.*",
                        ttyStr.apply("man_rotx"),
                        "ls",
                        ttyStr.apply("ls_tar"),
                        "cat code",
                        ttyStr.apply("cat_code"),
                        "cat.*",
                        ttyStr.apply("man_cat"),
                        "tar xvf .*",
                        ttyStr.apply("no_file"),
                        "cd.*",
                        ttyStr.apply("no_file")));
        var computtyTar = new EntityState(
                "computty_tar",
                ttyStr.apply("attack"),
                ttyStr.apply("inspect"),
                ttyStr.apply("interact"),
                computtyTarLogic);
        var computtyCdLogic = new TakeInput(
                "",
                TakeInput.makeCases(
                        "tar xvf code.tar$",
                        ...
                        ttyStr.apply("no_file"),
                        "tar.*",
                        ttyStr.apply("man_tar"),
                        "ls",
                        ttyStr.apply("ls_cd"),
                        "cat code.tar",
                        ttyStr.apply("cat_tar"),
                        "cat.*",
                        ttyStr.apply("man_cat"),
                        "cd.*",
                        ttyStr.apply("no_file"),
                        ...
                        "rotx.*",
                        ttyStr.apply("man_rotx")));
        var computtyCd = new EntityState(
                "computty_cd",
                ttyStr.apply("attack"),
                ttyStr.apply("inspect"),
                ttyStr.apply("interact"),
                computtyCdLogic);
        var computtyDefault = new TakeInput(
                "",
                TakeInput.makeCases(
    ...
    public String searchString(String stringId, String... roots) {
        for (String root : roots) {
            var attempt = Optional.ofNullable(this.language.get(root)).map(it -> it.get(stringId));
