...
public class DataWriterTests {
	...
	void pre() throws URISyntaxException {
		DataWriter.ACCOUNTS_PATH = Paths.get(getClass().getResource("/org/virtuoso/escape/model/data/accounts.json").toURI()).toString();
		DataWriter.GAMESTATES_PATH = Paths.get(getClass().getResource("/org/virtuoso/escape/model/data/gamestates.json").toURI()).toString();
	...
	void testWritingValidAccount(){
		...
		JSONObject dummyJSON = (JSONObject) accounts.get(GameState.instance().account().id().toString());
	...
	void testWritingEmptyAccount(){
		...
		JSONObject emptyDummyJSON = (JSONObject) DataLoader.loadAccounts().get(GameState.instance().account().id().toString());
	...
	void testWritingMultipleValidAccounts(){
		...
		assertEquals("3a77e04b2d960ac37396832feff4d1753cf29cd7", GameState.instance().account().hashedPassword());
	}
	...
	void testWritingValidGameState(){
		...
		assertTrue(DataLoader.loadGameStates().containsKey(GameState.instance().account().id().toString()));
		JSONObject mrsdummyGS = (JSONObject) DataLoader.loadGameStates().get(GameState.instance().account().id().toString());
		assertEquals(Difficulty.TRIVIAL, Difficulty.valueOf(mrsdummyGS.get("difficulty").toString()));
	}
